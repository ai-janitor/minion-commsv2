...
def _output(data: dict[str, object], human: bool = False) -> None:
    """Print result as JSON (default) or human-readable."""
    if "error" in data:
...
def register(ctx: click.Context, name: str, agent_class: str, model: str, description: str, transport: str) -> None:
    ...
    _output(_register(name, agent_class, model, description, transport), ctx.obj["human"])
...
def cold_start(ctx: click.Context, agent: str) -> None:
    ...
    _output(_cold_start(agent), ctx.obj["human"])
